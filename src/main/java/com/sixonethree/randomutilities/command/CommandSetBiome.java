		...
		if (args.length >= 1) {
			...
			for (int x = range * -1; x <= range; x ++) { // This is inefficient, determine whether or not the blocks are in the same biome and then set it
				...
				for (int z = range * -1; z <= range; z ++) {
					...
					byte[] oldBiome = player.worldObj.getChunkFromBlockCoords((int) player.posX + x, (int) player.posZ + z).getBiomeArray();
					...
					player.worldObj.getChunkFromBlockCoords((int) player.posX + x, (int) player.posZ + z).setBiomeArray(biomeChanged);
					player.worldObj.getChunkFromBlockCoords((int) player.posX + x, (int) player.posZ + z).setChunkModified();
				}
			}
