...
public class RecipesLunchboxFeeding implements IRecipe {
	...
	private ResourceLocation registryName = new ResourceLocation(Reference.MOD_ID, "recipe_lunchbox_feeding");
	...
	@Override public boolean matches(InventoryCrafting window, World world) {
		...
		if (l == 1 && f >= 1 && !s.isEmpty()) {
			...
			try {
				cast1 = (ILunchbox) s.getItem();
			} catch (ClassCastException e) {}
			try {
				cast2 = (IHeartCanister) s.getItem();
			} catch (ClassCastException e) {}
			float fs, mfs, hs, mhs;
			fs = mfs = hs = mhs = 0F;
			int c = -1;
