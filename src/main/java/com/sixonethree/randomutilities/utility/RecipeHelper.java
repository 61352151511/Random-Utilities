...
public class RecipeHelper {
	...
	private static void addRecipe(IRecipe recipe) {
		CraftingManager.func_193372_a(new ResourceLocation(Reference.MOD_ID, "recipe" + index ++), recipe);
	}
	
	private static ShapedRecipes buildShapedRecipe(ItemStack output, int w, int h, Object[] input) {
		if (w * h != input.length) throw new UnsupportedOperationException("Recipe input does not match dimensions");
	...
	private static NonNullList<Ingredient> buildInput(Object[] input) {
		...
		for (int i = 0; i < input.length; i ++) {
			...
			else list.add(i, Ingredient.field_193370_a);
		}
